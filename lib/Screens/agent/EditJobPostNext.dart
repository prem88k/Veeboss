...
import 'package:veeboss/Screens/agent/EditJobPostNextPage.dart';
...
import 'package:veeboss/Presentation/rounded_button.dart';
...
class EditJobPostNext extends StatefulWidget {
  File image;
  String title, category, vacancy, city, locality, gender;
  jobList jobListData;
  int id;
  EditJobPostNext(this.image,this.title, this.category, this.vacancy, this.city, this.locality, this.gender, this.jobListData, this.id);

  @override
  State<EditJobPostNext> createState() => _EditJobPostNextState();
}

class _EditJobPostNextState extends State<EditJobPostNext> {
  ...
  void initState() {
   ...
   /* print(_qualificawtion);
    print(widget.jobListData.qualification!);
    _selectedQualification.addAll(json.decode(widget.jobListData.qualification!));
    _selectedQualification.forEach((element) {
      _onJobQualification(element.replaceAll("[", "").replaceAll("]", ""));
    });
*/
    ...
    /*List<String> ch =  _qualification.map((category)  => widget.jobListData.qualification.toString()).toList();
    setState(() {
      _selectedQualification = ch;
      print(ch);
    });
*/
   /* if(widget.jobListData.qualification.toString() != null)
      {
       // _qualification.map((category) => widget.jobListData.qualification.toString()).toList();
        List<String> ch =  _qualification.map((category)  => widget.jobListData.qualification.toString()).toList();
        print(widget.jobListData.qualification.toString());
        print(ch);
      }*/

    if(widget.jobListData.shiftTiming.toString() != null)
     {
       int ch = _choices!.indexWhere((item) => widget.jobListData.shiftTiming.toString() == item);
      _choiceIndex = ch;
     }

    if(widget.jobListData.workingDays.toString() != null)
    {
      int wh = _wHours!.indexWhere((item) => widget.jobListData.workingDays.toString() == item);
      _wHoursIndex = wh;
    }
   // _wHours![_wHoursIndex] = widget.jobListData.workingDays.toString();

    if(widget.jobListData.workingDays != null)
    {
      _selectedStartTime = widget.jobListData.jobSTime;
    }

    if( widget.jobListData.jobETime.toString() != null)
    {
      _selectedEndTime = widget.jobListData.jobETime!;
    }
    super.initState();
  }
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) {
                            return EditJobPostNextPage(widget.title,widget.category, widget.vacancy, widget.city, widget.locality, widget.image, widget.gender,
  ...
  /*void checkValidation() {
    if (selectedMaxExp.isEmpty) {
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) {
                return CreateJobAgentNextPage1(widget.title,widget.category, widget.vacancy, widget.city, widget.locality, widget.image, widget.gender,
                    _choiceIndex != 2 ? _choices![_choiceIndex].toString() : _otherWorkingDays.text , _selectedEndTime.toString(),  _selectedStartTime.toString(),
                    _selectedjobBenefits.toString(), selectedMaxExp, selectedMinExp,
                    _qualification![__qualificationIndex].toString(), _wHoursIndex != 2 ? _wHours![_wHoursIndex].toString() : _otherWorkingHours.text, _selectedLanguage.toString()
                );
              }
          )
      );
