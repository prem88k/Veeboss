...
class EditPost extends StatefulWidget {
  int id;
  jobList jobListData;
  EditPost(this.id, this.jobListData );
...
class _EditPostState extends State<EditPost> {
  ...
  final TextEditingController _jobLocalityController = TextEditingController();
...
*/
  ...
  void initState() {
    print( widget.id);
    print(widget.jobListData.gender.toString());
    getPostData();
    getCategoryD();
    getCity();
    _gender = ["Any","Male","Female"];

    if(widget.jobListData.gender.toString() != null)
    {
      int gd = _gender!.indexWhere((item) => widget.jobListData.gender.toString() == item);
      _genderIndex = gd;
    }
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) {
                        return EditJobPostNext(_image!=null ? _image! : File(""), _titleController.text,
                            selectedCategory ,_noOfVacancyController.text,
                            selectedCity, selectedArea.text,
                            _gender![_genderIndex].toString(), widget.jobListData, widget.id);
                      },
                    ),
                  );
                },
 ...
 /* void checkValidation() {
  ...
  void getPostData() {
    ...
    widget.jobListData.post == null ?
    "-": _titleController.text = widget.jobListData.post!;

    widget.jobListData.vacancy!.toString() == null ?
     "-" : _noOfVacancyController.text = widget.jobListData.vacancy!.toString();
...
/*
Future<void> EditPostData() async {
    ...
    var uri = Uri.https(
      'yourang.shop',
      '/veeboss/public/api/v1/job/update',
    );
    final headers = {
      'x-session-token': prefs.getString('sessionToken')!,
    };
    Map<String, dynamic> body = {
      'post': _titleController.text,
      'description': _descriptionController.text,
      'vacancy': _noOfVacancyController.text,
    ...
    if (statusCode == 200) {
      ...
      Message(context, "Post Updated Successfully");
