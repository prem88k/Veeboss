...
class EditJobPostNextPage extends StatefulWidget {
  File image;
  String  title, category, vacancy, city, locality, gender, selectQualification, starTtime, endTime, jobBenefits, wDays, minExp, maxExp, choices, language;
  jobList jobListData;
  int id;
  EditJobPostNextPage(this.title, this.category, this.vacancy, this.city, this.locality,this.image, this.gender,
      this.choices, this.endTime, this.starTtime, this.jobBenefits, this.maxExp, this.minExp, this.selectQualification, this.wDays, this.language, this.jobListData, this.id);


  @override
  State<EditJobPostNextPage> createState() => _EditJobPostNextPageState();
}

class _EditJobPostNextPageState extends State<EditJobPostNextPage> {
  ...
  void initState() {
    ...
    if(widget.jobListData.shift.toString() != null)
       {
         int sh = _shiftTimings!.indexWhere((item) => widget.jobListData.shift.toString() == item);
         _shiftTimingsIndex = sh;
       }

    _communication_ctg = ["Excellent","Good", "Average"];
    if(widget.jobListData.communicationCategory.toString() != null)
      {
        int cc = _communication_ctg!.indexWhere((item) => widget.jobListData.communicationCategory.toString() == item);
        _communication_ctgIndex = cc;
      }

    getPostData();
    // _communication_ctg![_communication_ctgIndex] = widget.jobListData.communicationCategory.toString();
 ...
 /* void checkValidation() {
  ...
  Future<void> EditProductsApI() async {
    setState(() {
      isloading = true;
    });
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var url = Uri.https('yourang.shop', '/veeboss/public/api/v1/job/update');
    //----------------------------------------------------------
    ...
    request.fields['state'] = widget.locality;
    ...
    print(" Shift::::: ${_shiftTimingsIndex != 2 ? _shiftTimings![_shiftTimingsIndex].toString() : _otherShiftTimings.text}");
    ...
    request.send().then((response) {
        ...
        response.stream.transform(utf8.decoder).listen((value) {
          ...
          Message(context, "Post Edit Successfully");
  ...
  void getPostData() {
  setState(() {
  widget.jobListData.salary! == null ? "-" : _salaryController.text = widget.jobListData.salary!;
  _salaryEndController.text = widget.jobListData.salaryEnd!;
  _jobSkillController.text = widget.jobListData.skills!;
  _otherRequirementController.text = widget.jobListData.description!;
  _clientNameController.text = widget.jobListData.companyInfo!;
  _fullInterviewAddressController.text = widget.jobListData.companyAddress!;
  });
  }
}